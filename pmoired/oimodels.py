...
def Ssingle(oi, param, noLambda=False, allParams=None):
    ...
    lines = filter(lambda x: x.startswith("line_") and x.endswith("_f"), _param.keys())
    ...
    for l in lines:
        ...
        if "line_" + i + "_lorentzian" in _param.keys():
            ...
            f += (
                _param[l]
                * np.exp(-(np.abs(oi["WL"] - wl0) ** pow) / (dwl / 1000) ** pow)
                * (oi["WL"] >= wl0)
            )
            f += (
                _param[l]
                * np.exp(
                    -(np.abs(oi["WL"] - wl0) ** (2 * pow))
                    / (dwl / 1000 / 10) ** (2 * pow)
                )
                * (oi["WL"] < wl0)
            )
        elif "line_" + i + "_trunclor" in _param.keys():
            dwl = _param["line_" + i + "_trunclor"]  # in nm
            if dwl > 0:
                f += (
                    _param[l]
                    * (0.5 * dwl / 1000) ** 2
        ...
        elif "line_" + i + "_gaussian" in _param.keys():
            dwl = _param["line_" + i + "_gaussian"]  # in nm
            if "line_" + i + "_power" in _param.keys():
                _pow = _param["line_" + i + "_power"]
            else:
                _pow = 2.0
            _tmp = _param[l] * np.exp(
                -(np.abs(oi["WL"] - wl0) ** _pow) / (2 * (dwl / 1000 / 2.35482) ** _pow)
            )
    ...
    if len(sw) > 0:
        assert len(sw) == len(sf), (
            "cubic spline flux is ill defined: N(fspl_wl)!=N(fspl_f)"
        )
    ...
    if len(sw) > 0:
        ...
        if "fit" in oi and "wl ranges" in oi["fit"]:
            assert len(oi["fit"]["wl ranges"]) == 1, (
                "wavelet flux only work for single 'wl ranges'"
            )
            X = np.linspace(
                max(oi["fit"]["wl ranges"][0][0], min(oi["WL"])),
                min(oi["fit"]["wl ranges"][0][1], max(oi["WL"])),
                len(sw),
            )
        else:
    ...
    if "spectrum" in _param.keys():
        ...
        if not allParams is None:
            Cs = set([x.split(",")[0] for x in allParams if "," in x])
            for c in Cs:
                if "#" + c + ",SPECTRUM" in sp:
                    tmpp = {x: allParams[x] for x in allParams if not "," in x}
                    tmpp |= {
                        x.split(c + ",")[1]: allParams[x]
                        for x in allParams
                        if x.startswith(c + ",")
                    }
                    # print('#'+c+',SPECTRUM' in sp)
                    sp = sp.replace(
                        "#" + c + ",SPECTRUM",
                        "np.array(" + str(list(Ssingle(oi, tmpp))) + ")",
                    )
        try:
            f += eval(sp)
        except:
...
def _Kepler3rdLaw(a=None, P=None, M1M2=None):
    ...
    eC = 0.0075
    if a is None:
        return (P**2 * M1M2 * C) ** (1 / 3.0)
    elif P is None:
        return (a**3 / (M1M2 * C)) ** (1 / 2.0)
    elif M1M2 is None:
        return a**3 / (P**2 * C)
    else:
...
def _ti2campbell(ti, c=None, deg=True):
    ...
    if not c is None:
        print(
            "opO: %.3f (%.3f) delta=%.3f"
            % (opO, c["omega"] + c["OMEGA"], opO - c["omega"] - c["OMEGA"])
        )
        print(
            "   sign(B-F)=%2.0f sign(sin(opO))=%2.0f"
            % (np.sign(ti["B"] - ti["F"]), np.sign(np.sin(opO)))
        )
    ...
    if not c is None:
        print(
            "omO: %.3f (%.3f) delta=%.3f"
            % (omO, c["omega"] - c["OMEGA"], omO - c["omega"] + c["OMEGA"])
        )
        print(
            "   sign(-B-F)=%2.0f sign(sin(omO))=%2.0f"
            % (np.sign(-ti["B"] - ti["F"]), np.sign(np.sin(omO)))
        )
    ...
    if not c is None:
        ...
        print(
            "  %swith d3/d1: %.3f"
            % ("*" if d1 >= d2 else " ", 2 * np.arctan(np.sqrt(d3 / d1)))
        )
        print(
            "  %swith d4/d2: %.3f"
            % (" " if d1 >= d2 else "*", 2 * np.arctan(np.sqrt(d4 / d2)))
        )
...
def _campbell2ti(c):
    ...
    return {
        "A": c["a"]
        * (
            np.cos(c["omega"]) * np.cos(c["OMEGA"])
            - np.sin(c["omega"]) * np.sin(c["OMEGA"]) * np.cos(c["incl"])
        ),
        "B": c["a"]
        * (
            np.cos(c["omega"]) * np.sin(c["OMEGA"])
            + np.sin(c["omega"]) * np.cos(c["OMEGA"]) * np.cos(c["incl"])
        ),
        "F": -c["a"]
        * (
            np.sin(c["omega"]) * np.cos(c["OMEGA"])
            + np.cos(c["omega"]) * np.sin(c["OMEGA"]) * np.cos(c["incl"])
        ),
        "G": -c["a"]
        * (
            np.sin(c["omega"]) * np.sin(c["OMEGA"])
            - np.cos(c["omega"]) * np.cos(c["OMEGA"]) * np.cos(c["incl"])
        ),
    }
...
def _orbit(t, P, Vrad=False, verbose=False, withZ=False):
    ...
    if "M" in P and "P" in P:
        a_au = (P["M"] * (P["P"] / 365.25) ** 2) ** (1 / 3.0)  # in AU
    ...
    if not "K" in P and (not "Ka" in P and not "Kb" in P) and not a_au is None:
        P["K"] = (
            ...
            * a_au
            * 1.495978707e8
            * np.sin(P["incl"] * np.pi / 180)
            / (P["P"] * 24 * 3600 * np.sqrt(1 - np.abs(P["e"]) ** 2))
        )  # km/s
    ...
    if "a" in P:
        ...
        if newOrbitalParameters:
            ...
            x = r * (
                np.cos(_O) * np.cos(nu + _o) - np.sin(_O) * np.sin(nu + _o) * np.cos(_i)
            )
            y = r * (
                np.sin(_O) * np.cos(nu + _o) + np.cos(_O) * np.sin(nu + _o) * np.cos(_i)
            )
            ...
            x, y, z = (
                x * np.cos(_O - np.pi / 2) + y * np.sin(_O - np.pi / 2),
    ...
    if "Ka" in P and "Kb" in P:
        ...
        VA = -P["Ka"] * (
            np.cos((P["omega"]) * np.pi / 180 + nu)
            + np.abs(P["e"]) * np.cos((P["omega"]) * np.pi / 180)
        )
        VB = P["Kb"] * (
            np.cos((P["omega"]) * np.pi / 180 + nu)
            + np.abs(P["e"]) * np.cos((P["omega"]) * np.pi / 180)
...
def _expandVec(x, n: int, dx=None):
    ...
    if dx is None:
        res = np.interp(
            np.linspace(0, 1, (n + 1) * (len(x) - 1) + 1), np.linspace(0, 1, len(x)), x
        )
...
def VsingleOI(
    ...
    if "OI_FLUX" in oi:
        ...
        for k in oi["OI_FLUX"]:
            ...
            res["OI_FLUX"][k]["MJD2"] = (
                oi["OI_FLUX"][k]["MJD"][:, None] + 0 * res["WL"][None, :]
            )
    ...
    if "OI_T3" in oi:
        ...
        for k in oi["OI_T3"]:
            ...
            res["OI_T3"][k]["MJD2"] = (
                oi["OI_T3"][k]["MJD"][:, None] + 0 * res["WL"][None, :]
            )
            res["OI_T3"][k]["FLAG"] = np.bool(0.0 * res["OI_T3"][k]["MJD2"])
            res["OI_T3"][k]["u1/wl"] = (
                oi["OI_T3"][k]["u1"][:, None] / res["WL"][None, :]
            )
            res["OI_T3"][k]["v1/wl"] = (
                oi["OI_T3"][k]["v1"][:, None] / res["WL"][None, :]
            )
            res["OI_T3"][k]["u2/wl"] = (
                oi["OI_T3"][k]["u2"][:, None] / res["WL"][None, :]
            )
            res["OI_T3"][k]["v2/wl"] = (
                oi["OI_T3"][k]["v2"][:, None] / res["WL"][None, :]
            )
    ...
    if timeit:
        print(
            " " * indent + "VsingleOI > spectrum %.3fms" % (1000 * (time.time() - t0))
        )
    ts = time.time()
    ...
    if "projang" in _param.keys() and "incl" in _param.keys():
        ...
        if not I is None:
            if len(baselines):
                _X = (
                    np.cos(rot) * (X - np.mean(x(res[key][baselines[0]])))
                    + np.sin(rot) * (Y - np.mean(y(res[key][baselines[0]])))
                ) / np.cos(_param["incl"] * np.pi / 180)
                _Y = -np.sin(rot) * (X - np.mean(x(res[key][baselines[0]]))) + np.cos(
                    rot
                ) * (Y - np.mean(y(res[key][baselines[0]])))
            else:
                _X = (np.cos(rot) * (X - _xs) + np.sin(rot) * (Y - _ys)) / np.cos(
                    _param["incl"] * np.pi / 180
                )
        ...
        if not I is None:
            if len(baselines):
                _X, _Y = (
                    X - np.mean(x(res[key][baselines[0]])),
                    Y - np.mean(y(res[key][baselines[0]])),
                )
            else:
                _X, _Y = X - _xs, Y - _ys
    ...
    if "sparse" in _param:
        ...
        if du:  # -- slanted
            Vfdu = lambda z: (
                2
                * scipy.special.j1(_c * _param["ud"] * _Bdu(z))
                / (_c * _param["ud"] * _Bdu(z))
            )
            Vfdv = lambda z: (
                2
                * scipy.special.j1(_c * _param["ud"] * _Bdv(z))
                / (_c * _param["ud"] * _Bdv(z))
            )
        ...
        if np.abs(_param["fwhm"]) > 1e-3:  # UGLY KLUDGE!
            a = 1.0 / (2.0 * (_param["fwhm"] / (2 * np.sqrt(2 * np.log(2)))) ** 2)
        ...
        ain = 1.0 / (2.0 * (fwhmin / (2 * np.sqrt(2 * np.log(2)))) ** 2)
        aou = 1.0 / (2.0 * (fwhmout / (2 * np.sqrt(2 * np.log(2)))) ** 2)
        ...
        Vf = lambda z: (
            (
                np.exp(-2j * _c * _uwl(z) * _offXo - 2j * _c * _vwl(z) * _offYo)
                * _crout**2
                * 2
                * scipy.special.j1(_c * _crout * _Bwl(z) + 1e-12)
                / (_c * _crout * _Bwl(z) + 1e-12)
                - np.exp(-2j * _c * _uwl(z) * _offXi - 2j * _c * _vwl(z) * _offYi)
                * _crin**2
                * 2
                * scipy.special.j1(_c * _crin * _Bwl(z) + 1e-12)
        ...
        if du:  # --slanted
            if len(_n):
                print(
                    "WARNING: slanted disk with azimutal variation not implemented properly!"
                )
            Vfdu = lambda z: _Vazvar(
                _udu(z),
                _vdu(z),
                Ir,
                _r,
        ...
        if not I is None:
            ...
            I = _Vazvar(
                None, None, Ir, _r, _n, _phi, _amp, stretch=stretch, numerical=1, XY=XY
            )
            if np.sum(I) == 0:
                # -- unresolved -> single imPixel
                R2 = _X**2 + _Y**2
        ...
        for _mjd in sorted(set(np.int_(res["MJD"] + 0.5))):
            MJD.append(
                np.round(np.mean(res["MJD"][np.int_(res["MJD"] + 0.5) == _mjd]), 2)
            )
        # try:
        ...
        Vf = lambda z: VsparseImage(
            z["u"], z["v"], res["WL"][wwl], sparse_param, z["MJD"]
        )
        if not I is None:
    ...
    if du and not I is None:
        ...
        I *= (
            1.0
            + np.sin(_param["slant projang"] * np.pi / 180)
            * _param["slant"]
            / Rout
            * _X
            + np.cos(_param["slant projang"] * np.pi / 180)
            * _param["slant"]
            / Rout
            * _Y
        )
    ...
    for k in baselines:  # -- for each baseline
        ...
        V = np.zeros(
            (res[key][k]["u/wl"].shape[0], res[key][k]["u/wl"].shape[1]), dtype=complex
        )
        if du:  # -- for slanted
            V[:, wwl] = Vf(oi[key][k])
    ...
    if timeit:
        print(
            " " * indent
            + "VsingleOI > complex vis %.3fms" % (1000 * (time.time() - tv))
        )
    if "OI_T3" in oi.keys() and not noT3:
        ...
        for k in oi["OI_T3"].keys():
            ...
            for l in [
                ...
                "v2",
                "MJD",
                "formula",  #'FLAG', 'Bmax/wl', 'Bmin/wl', 'Bavg/wl', 'MJD2',
                "B1",
                "B2",
                "B3",
            ]:
        ...
        if timeit:
            print(
                " " * indent
                + "VsingleOI > T3 from V %.3fms" % (1000 * (time.time() - t3))
            )
    ...
    if not I is None:
        ...
        if len(I.shape) == 2:
            ...
            res["MODEL"] = {"cube": I, "X": X, "Y": Y, "image": np.mean(I, axis=0)}
    ...
    if "OI_FLUX" in res:
        for k in res["OI_FLUX"]:
            ...
            res["OI_FLUX"][k]["FLUX"] = (
                0 * res["OI_FLUX"][k]["MJD"][:, None] + flux[None, :]
            )
...
def VsparseImage(
    ...
    vis = np.exp(
        ...
        * np.pi
        * c
        * (u[:, None, None] * _x[None, None, :] + v[:, None, None] * _y[None, None, :])
        / wl[None, :, None]
    )
    vis = np.sum(vis * _sparse_image["I"][None, None, :] ** pow, axis=2) / np.sum(
        _sparse_image["I"] ** pow
    )
                ...
                for dx in DX:
                    for dy in DY:
                        if (
                            ...
                            _w[(dx, dy)] = 1 - np.abs(
                                (_x[k] - X[i + dx]) * (_y[k] - Y[j + dy])
                            ) / ((imFov / (imN - 1)) ** 2)
                for dxy in _w:
                    image[j + dxy[1], i + dxy[0]] += (
                        _sparse_image["I"][k] ** pow * _w[dxy] / 3
                    )
            else:
                # -- gaussian
                _w = {}
                for dx in DX:
                    for dy in DY:
                        if (
                            ...
                            _w[(dx, dy)] = np.exp(
                                -((_x[k] - X[i + dx]) ** 2 + (_y[k] - Y[j + dy]) ** 2)
                                / (imFov / (imN - 1)) ** 2
                            )
                nor = np.sum([_w[dxy] for dxy in _w])
                for dxy in _w:
                    image[j + dxy[1], i + dxy[0]] += (
                        _sparse_image["I"][k] ** pow * _w[dxy] / nor
                    )
...
def Vkepler(
    ...
    reso = (
        np.min(1e-6 * wl[None, :] / np.sqrt(u**2 + v**2)[:, None])
        * 180
        * 3600
        * 1000
        / np.pi
    ...
    assert Rout > 0, (
        "Rout, diamout or fwhm must be defined: "
        + "Rout="
        + str(Rout)
        + " fwhm="
        + str(fwhm)
    ...
    cp, sp = (
        np.cos(param["projang"] * np.pi / 180 - np.pi / 2),
        np.sin(param["projang"] * np.pi / 180 - np.pi / 2),
    )

    # -- non uniform coverage in "r" (more points at inner radius, where range of velocity is widest)
    R = (
        np.linspace(Rin ** (1 / _p), Rout ** (1 / _p), max(int((Rout - Rin) / drin), 3))
        ** _p
    )
    dR = np.gradient(R)
    R = (
        np.linspace(
            Rin ** (1 / _p),
            Rout ** (1 / _p),
            max(int(dR[0] / drin * (Rout - Rin) / drin), 3),
        )
        ** _p
    )
    dR = np.gradient(R)
    ...
    loren = lambda x, wl0, dwl: (
        (0.5 * dwl / 1000) ** 2 / ((x - wl0) ** 2 + (0.5 * dwl / 1000) ** 2)
    )
    gauss = lambda x, wl0, dwl: np.exp(
        -((x - wl0) ** 2) / (2 * dwl / 1000 / 2.35482) ** 2
    )
    ...
    for a in As:  # for each spectral lines
        ...
        if (
            a.replace("wl0", "azamp1") in param
            and a.replace("wl0", "azprojang1") in param
        ):
        ...
        if a.replace("wl0", "gaussian") in param:
            flux += amp[None, :] * gauss(
                wl[:, None] * (1 - P[:, 5][None, :] / 2.998e5),
                param[a],
                param[a.replace("wl0", "gaussian")],
            )
        elif a.replace("wl0", "lorentzian") in param:
            flux += amp[None, :] * loren(
                wl[:, None] * (1 - P[:, 5][None, :] / 2.998e5),
                param[a],
                param[a.replace("wl0", "lorentzian")],
            )
        else:
            # -- default: gaussian, based on spectral resolution
            flux += amp[None, :] * gauss(
                wl[:, None] * (1 - P[:, 5][None, :] / 2.998e5), param[a], dwl
            )
...
def VfromImageOI(oi):
    ...
    if not "MODEL" in oi.keys() and not (
        "image" in oi["MODEL"].keys() or "cube" in oi["MODEL"].keys()
    ):
    ...
    for k in oi["OI_VIS"].keys():
        ...
        phi = (
            ...
            * _c
            * (
                oi["MODEL"]["X"][None, None, :, :] * tmp["u/wl"][:, :, None, None]
        ...
        if "cube" in oi["MODEL"].keys():
            V = (
                np.sum(oi["MODEL"]["cube"][None, :, :, :] * np.exp(phi), axis=(2, 3))
                / np.sum(oi["MODEL"]["cube"], axis=(1, 2))[None, :]
            )
        else:
            V = np.sum(
                oi["MODEL"]["image"][None, None, :, :] * np.exp(phi), axis=(2, 3)
...
def VmodelOI(
    ...
    if len(comp) == 0:
        ...
        if (
            ...
            if timeit:
                print(
                    " " * indent
                    + "VmodelOI > dPHI %.3fms" % (1000 * (time.time() - t0))
                )
                t0 = time.time()
        ...
        if n < 2:
            ...
            for k in kz:
                for x in np.linspace(-0.5, 0.5, n + 2)[1:-1]:
                    tmp[
                        c + "&dwl%.7f&ffrac%.7f," % (x * dwl, 1 / n) + k.split(",")[1]
                    ] = param[k]
    if debug:
    ...
    for c in comp:
        ...
        if res == {}:
            ...
            if "&dwl" in c:
                # -- TODO: flux(MJD)
                res["MODEL"][c.split("&dwl")[0] + ",flux"] = res["MODEL"][
                    "totalflux"
                ].copy()
                res["MODEL"][c.split("&dwl")[0] + ",negativity"] = res["MODEL"][
                    "negativity"
                ]
            ...
            for k in res["MODEL"][c + ",vis"].keys():  # for each baseline
                ...
                if "totalflux(MJD)" in res["MODEL"]:
                    tmp = res["MODEL"]["totalflux(MJD)"].replace(
                        "$WL", "res['MODEL']['WL']"
                    )
            ...
            if "image" in m["MODEL"].keys():
                ...
                res["MODEL"]["image"] += (
                    np.mean(m["MODEL"]["totalflux"]) * m["MODEL"]["image"]
                )
                if "cube" in m["MODEL"]:
            ...
            for k in res["MODEL"][c + ",vis"].keys():  # for each baseline
                if "totalflux(MJD)" in res["MODEL"]:
                    tmp = m["MODEL"]["totalflux(MJD)"].replace(
                        "$WL", "m['MODEL']['WL']"
                    )
        ...
        if timeit:
            print(
                " " * indent
                + 'VmodelOI > VsingleOI "%s" %.3fms' % (c, 1000 * (time.time() - tc))
            )
    ...
    for b in res["MOD_VIS"].keys():
        ...
        if "OI_VIS" in res and b in res["OI_VIS"]:
            ...
            res["OI_VIS"][b]["PHI"] = (
                np.angle(res["MOD_VIS"][b]) * 180 / np.pi + 180
            ) % 360 - 180
    ...
    if "OI_FLUX" in oi.keys():
        for k in oi["OI_FLUX"].keys():
            if "totalflux(MJD)" in res["MODEL"]:
                ...
                totalflux = (
                    res["OI_FLUX"][k]["FLUX"] * 0 + res["MODEL"]["totalflux"][None, :]
                )
...
def _injectFeatures(oi, truth, inject):
    ...
    for e in E:  # for each extension
        if not e in oi:
            continue
        tmp = {}
        for k in oi[e]:  # for each observable
            tmp[k] = {}
            for o in oi[e][k]:  # for each telescope / baseline / triangle
                if o in E[e]:
                    # -- observations - bestmodel + bestmodel_with_injection
        ...
        for k in sorted(list(paramsI.keys()), key=lambda x: -len(x)):
            if type(paramsI[k]) == str:
                ...
                if compute and not loop and tmp != "orbit":
                    if s + "MJD" in tmp and not (
                        k in ["x", "y"] or any([t in k for t in [",x", ",y"]])
                    ):
                        # print('MJD!')
                        if MJD == 0:
        ...
        for k in _param.keys():
            if "line_" in k and "wl0" in k:
                ...
                if k.replace("_wl0", "_lorentzian") in _param.keys():
                    dwl = 3 * _param[k.replace("wl0", "lorentzian")] / 1000.0
                ...
                if kv in _param:
                    ...
                    if kv in _param:
                        if "Rin" in _param:
                            vel = _param[kv] / np.sqrt(
                                _param[kv.replace("V1mas", "Rin")]
                            )
                        elif "diamin" in _param:
                            vel = _param[kv] / np.sqrt(
                                0.5 * _param[kv.replace("V1mas", "diamin")]
                            )

                dwl = np.sqrt(dwl**2 + (_param[k] * vel / 2.998e5) ** 2)
                if k.replace("_wl0", "_truncexp") in _param.keys():
                    dwl = 2 * _param[k.replace("wl0", "truncexp")] / 1000.0
                    w *= ~(
                        ((oi["WL"] - _param[k]) <= 1.3 * dwl)
    ...
    for k in oi["OI_VIS"].keys():
        ...
        for i, phi in enumerate(oi["OI_VIS"][k]["PHI"]):
            ...
            if _checkEmpty and np.sum(mask) == 0:
                print(
                    k,
                    "empty mask! -> w=%.0f, flag=%.0f"
                    % (np.sum(w), np.sum(~oi["OI_VIS"][k]["FLAG"][i, :])),
                    end=" ",
                )
            ...
            if "EPHI" in oi["OI_VIS"][k] or "NAIVE_EPHI" in oi["OI_VIS"][k]:
                ...
                if (
                    "max relative error" in oi["fit"]
                    and "DPHI" in oi["fit"]["max relative error"]
                ):
                    # -- ignore data with large error bars
                    mask *= err <= (
                        oi["fit"]["max relative error"]["DPHI"]
                ...
                if (
                    "min relative error" in fit
                    and "DPHI" in oi["fit"]["min relative error"]
                ):
                    # -- force error to a minimum value
                    err = np.maximum(
                        oi["fit"]["min relative error"]["DPHI"]
                        * np.abs(oi["OI_VIS"][k]["PHI"][i, :]),
                        err,
                    )
                # -- ignore 0-errors
        ...
        if visamp:
            ...
            for i, vis in enumerate(oi["OI_VIS"][k]["|V|"]):
                ...
                if "E|V|" in oi["OI_VIS"][k] or "NAIVE_E|V|" in oi["OI_VIS"][k]:
                    ...
                    if (
                        "max relative error" in oi["fit"]
                        and "N|V|" in oi["fit"]["max relative error"]
                    ):
                        # -- ignore data with large error bars
                        vmask *= verr < (
                            oi["fit"]["max relative error"]["N|V|"]
                    ...
                    if (
                        "min relative error" in fit
                        and "N|V|" in oi["fit"]["min relative error"]
                    ):
                        # -- force error to a minimum value
                        verr = np.maximum(
                            oi["fit"]["min relative error"]["N|V|"]
                            * np.abs(oi["OI_VIS"][k]["|V|"][i, :]),
                            verr,
                        )
                    vmask *= verr > 0
                ...
                if np.sum(vmask) > vorder:
                    if not verr is None:
                        c = np.polyfit(
                            oi["WL"][vmask], vis[vmask], vorder, w=1 / verr[vmask]
                        )
                    else:
                        c = np.polyfit(oi["WL"][vmask], vis[vmask], vorder)
    ...
    for k in oi["OI_VIS2"].keys():
        ...
        if vis2amp:
            ...
            for i, vis in enumerate(oi["OI_VIS2"][k]["V2"]):
                ...
                if "EV2" in oi["OI_VIS2"][k]:
                    ...
                    if (
                        "max relative error" in oi["fit"]
                        and "NV2" in oi["fit"]["max relative error"]
                    ):
                        # -- ignore data with large error bars
                        vmask *= verr < (
                            oi["fit"]["max relative error"]["NV2"]
                    ...
                    if (
                        "min relative error" in fit
                        and "NV2" in oi["fit"]["min relative error"]
                    ):
                        # -- force error to a minimum value
                        verr = np.maximum(
                            oi["fit"]["min relative error"]["NV2"]
                            * np.abs(oi["OI_VIS2"][k]["V2"][i, :]),
                            verr,
                        )
                    vmask *= verr > 0
                ...
                if np.sum(vmask) > v2order:
                    if not verr is None:
                        c = np.polyfit(
                            oi["WL"][vmask], vis[vmask], v2order, w=1 / verr[vmask]
                        )
                    else:
                        c = np.polyfit(oi["WL"][vmask], vis[vmask], v2order)
                    vdata.append(vis / np.polyval(c, oi["WL"]))
                else:
                    # -- not polynomial fit, use median
                    if np.median(vis) != 0:
        ...
        if vis2amp:
            ...
            if "EV2" in oi["OI_VIS2"][k]:
                ...
                oi["DVIS2"][k]["ENV2"] = (
                    np.array(edata)[:, None] + 0 * oi["OI_VIS2"][k]["EV2"]
                )
...
def computeNormFluxOI(oi, param=None, order="auto", debug=False):
    ...
    if (
        "fit" in oi
        and "continuum ranges" in oi["fit"]
        and oi["fit"]["continuum ranges"] != []
    ):
        wc = np.zeros(oi["WL"].shape)
        for WR in oi["fit"]["continuum ranges"]:
            wc += (oi["WL"] >= WR[0]) * (oi["WL"] <= WR[1])
    ...
    if "MODEL" in oi.keys() and "totalflux" in oi["MODEL"].keys():
        ...
        for k in list(oi["MODEL"].keys()):
            if k.endswith(",flux"):
                oi["MODEL"][k.replace(",flux", ",nflux")] = oi["MODEL"][k] / np.polyval(
                    c, oi["WL"]
                )
...
def computeT3fromVisOI(oi):
    ...
    if "IM_VIS" in oi.keys():
        ...
        for k in oi["OI_T3"].keys():
            ...
            if np.isscalar(s[0]):
                oi["IM_T3"][k]["T3PHI"] = (
                    s[0] * oi["IM_VIS"][t[0]]["PHI"][w0, :]
                    + s[1] * oi["IM_VIS"][t[1]]["PHI"][w1, :]
                    + s[2] * oi["IM_VIS"][t[2]]["PHI"][w2, :]
                )
            else:
                oi["IM_T3"][k]["T3PHI"] = (
                    s[0][:, None] * oi["IM_VIS"][t[0]]["PHI"][w0, :]
...
def residualsOI(
    ...
    for f in fit["obs"]:
        ...
        if f in sorted(ext.keys()):
            ...
            for k in sorted(
                ...
                test = testTelescopes(k, ignoreTelescope) or testBaselines(
                    k, ignoreBaseline
                )
                if not test:
                    ...
                    if (
                        "max relative error" in oi["fit"]
                        and f in oi["fit"]["max relative error"]
                    ):
                        # -- ignore data with large error bars
                    ...
                    if (
                        "min relative error" in fit
                        and f in oi["fit"]["min relative error"]
                    ):
                        # -- force error to a minimum value
                        err = np.maximum(
                            oi["fit"]["min relative error"][f]
                            * np.abs(oi[ext[f]][k][f]),
                            err,
                        )
                    ...
                    if ext[f] in oi and ext[f] in m:
                        ...
                        if what:
                                ...
                                if k != "all":
                                    kk = "|".join(
                                        [
                                            ...
                                            % (
                                                "+"
                                                if m[ext[f]][k]["formula"][0][ii] == 1
                                                else "-",
                                                m[ext[f]][k]["formula"][1][ii],
                                            )
                                            for ii in range(3)
                                        ]
                                    )
                            ...
                            if not type(kk) is list:
                                wh += [
                                    f + ":" + kk + ";MJD:%.6f" % x
                                    for x in oi[ext[f]][k]["MJD2"][mask].flatten()
                                ]
                            else:
                                for j, _k in enumerate(kk):
                                    wh.append(
                                        ...
                                        + _k
                                        + ";MJD:%.6f"
                                        % oi[ext[f]][k]["MJD2"][mask].flatten()[j]
                                    )
                    else:
    ...
    if "fit" in oi and "prior" in oi["fit"]:
        ...
        # else:
        tmp = computePriorL(
            computeLambdaParams(param, MJD=np.mean(oi["MJD"])),
            oi["fit"]["prior"],
            weight=np.sqrt(len(res)),
        )
    ...
    if "additional residuals" in param:
        tmp = param["additional residuals"](
            computeLambdaParams(param, MJD=np.mean(oi["MJD"]))
        )
        res = np.append(res, tmp)
        if fullOutput:
...
def sparseFitOI(
    ...
    while test:
        print(
            "Sparse fit %s: running fit #%d, for %d parameters"
            % (time.asctime(), N, len(fitOnly))
        )
        fit = fitOI(
            oi,
            firstGuess,
        ...
        if len(fit["not significant"]):
            print(
                "  WARNING, ["
                + ",".join(fit["not significant"])
                + "] do not change chi2"
            )
            for s in fit["not significant"]:
                fitOnly.remove(s)
            print(
                "Sparse fit %s: re-running fit #%d, for %d parameters"
                % (time.asctime(), N, len(fitOnly))
            )
            fit = fitOI(
                oi,
...
def sparseFitFluxes(
    ...
    for k in N:
        assert N[k] >= 4 and 2 ** int(np.log2(N[k])) == N[k], (
            "N[%s]>=4 must be power of 2" % k
        )
    if not refFlux is None:
        assert refFlux in N, (
            "refFlux must be one of the components with wavelet spectra"
        )
    ...
    f = (
        100
        * sum(["wvl" in k and fit["uncer"][k] > 0 for k in fit["uncer"]])
...
def fitOI(
    ...
    if randomise is False:
        ...
        if onlyMJD:
            ...
            onlyMJD = None
        tmp = randomiseData2(oi, verbose=False, keepFlux=keepFlux, onlyMJD=onlyMJD)
    ...
    if (
        type(verbose) == int
        and verbose >= 2
        and len(fit["cord"].keys()) > 1
        and any([fit["uncer"][k] > 0 for k in fit["uncer"]])
    ):
        dpfit.dispCor(fit)
    if lowmemory:
...
def _updateAzAmpsProjangs(params):
    ...
    if len(comps) > 0:
        ...
        for k in params:
            if ",az amp" in k:
                ...
                if (
                    type(params[k]) == str
                    or type(params[k.replace("az amp", "az projang")]) == str
                ):
                    _safe[c] = False
                for kp in params:
                    if type(params) == str and (
                        "$" + k in params
                        or "$" + k.replace("az amp", "az projang") in params
                    ):
                        _safe[c] = False
    ...
    for k in params:
        if safe(k) and "az amp" in k:
            if (
                type(params[k]) != str
                and type(params[k.replace("az amp", "az projang")]) != str
                and params[k] < 0
            ):
...
def randomiseData2(oi, verbose=False, keepFlux=False, onlyMJD=None):
    ...
    for i in range(len(oi) * 2):
        ...
        for k in tmp["configurations per MJD"].keys():
            mjd_c.extend(
                ["%.5f" % k + "|" + str(c) for c in tmp["configurations per MJD"][k]]
            )
        mjd_c = list(set(mjd_c))

        if not onlyMJD is None:
            ignore = list(
                filter(lambda f: any(["%.5f" % x in f for x in onlyMJD]), mjd_c)
            )
        else:
        ...
        if keepFlux:
            ...
            exts = list(
                filter(
                    lambda x: x in ["OI_VIS", "OI_VIS2", "OI_T3", "OI_FLUX", "NFLUX"],
                    tmp.keys(),
                )
            )
...
# if time.gmtime().tm_mon==6:
#     _prog_bloc, _prog_wblock = '\U0001F3F3\U0000FE0F\U0000200D\U0001F308', 1.6
# elif time.gmtime().tm_mon==10:
#     _prog_bloc, _prog_wblock = '\U0001F383', 1.5
# elif True or time.gmtime().tm_mon==12:
#     _prog_bloc, _prog_wblock = '\U000026C4', 1.6
# else:
_prog_bloc, _prog_wblock = bytes((219,)).decode("cp437"), 1


def progress(results=None, finish=False):
    global _prog_N, _prog_last, _prog_bloc, _prog_wbloc
    if finish:
        _prog_N = _prog_Nmax
    ...
    if finish or time.time() - _prog_last >= PROG_UPDATE:
        ...
        res = (
            ...
            + ": "
            + "["
            + _prog_bloc * int(_nb * _prog_N / max(_prog_Nmax, 1) / _prog_wblock)
            + "." * (_nb - int(_nb * _prog_N / max(_prog_Nmax, 1)))
            + "]"
            + fmt % (_prog_N, _prog_Nmax, tleft)
            + "\r"
        )
        # print(res)
...
def gridFitOI(
        ...
        for k in expl["grid"]:
            ...
            for l in expl["grid"]:
                R[l] = (
                    R[l][:, None]
                    + (k == l)
                    * np.linspace(expl["grid"][k][0], expl["grid"][k][1], n)[None, :]
                )
                R[l] = R[l].flatten()
    ...
    for i in range(N):
        ...
        if "rand" in expl:
            for k in expl["rand"]:
                tmp[k] = expl["rand"][k][0] + np.random.rand() * (
                    expl["rand"][k][1] - expl["rand"][k][0]
                )
        if "randn" in expl:
            for k in expl["randn"]:
    ...
    if len(PARAM) < N and verbose:
        print(
            N - len(PARAM),
            "grid point%s not within constraints"
            % ("" if (N - len(PARAM)) == 1 else "s"),
        )
    ...
    if verbose:
        print(
            time.asctime()
            + ": it took %.1fs, %.2fs per fit on average"
            % (time.time() - t, (time.time() - t) / N),
            end=" ",
        )
...
def analyseGrid(fits, expl, debug=False, verbose=1, deltaChi2=None):
    ...
    if debug or verbose:
        ...
        if len(bad) - len(errTooLarge):
            print(
                " ",
                len(bad) - len(errTooLarge) - len(chi2TooLarge),
                "did not numerically converge",
        ...
        if len(chi2TooLarge):
            print(
                ...
                len(chi2TooLarge),
                "have chi2r > %.3f + %f = %.3f"
                % (chi2min, deltaChi2, chi2min + deltaChi2),
            )
...
def showGrid(
    ...
    if not significance is False:
        c = np.array(
            [_nSigmas(significance, r["chi2"], r["ndof"]) for r in res if ~r["bad"]]
        )
        color = r"significance ($\sigma$)"
    else:
    ...
    if type(expl) is dict and "grid" in expl and px in expl["grid"]:
        dx = expl["grid"][px][2]
        IX = np.linspace(
            expl["grid"][px][0] - expl["grid"][px][2] / 2,
            expl["grid"][px][1] + expl["grid"][px][2] / 2,
            S
            * int(
                (expl["grid"][px][1] - expl["grid"][px][0]) / expl["grid"][px][2] + 2
            ),
        )
    else:
        dx = np.ptp(ix) / np.sqrt(len(ix))
        IX = np.linspace(
            np.min(ix) - dx / 2, np.max(ix) + dx / 2, S * int(np.ptp(ix) / dx + 1)
        )

    if type(expl) is dict and "grid" in expl and py in expl["grid"]:
        dy = expl["grid"][py][2]
        IY = np.linspace(
            expl["grid"][py][0] - expl["grid"][py][2] / 2,
            expl["grid"][py][1] + expl["grid"][py][2] / 2,
            S
            * int(
                (expl["grid"][py][1] - expl["grid"][py][0]) / expl["grid"][py][2] + 2
            ),
        )
    else:
        dy = np.ptp(iy) / np.sqrt(len(iy))
        IY = np.linspace(
            np.min(iy) - dy / 2, np.max(iy) + dy / 2, S * int(np.ptp(iy) / dy + 1)
        )
    ...
    if not (type(interpolate) == int and interpolate > 1):
        plt.scatter(x, y, c=c, vmin=vmin, vmax=vmax, cmap=cmap, plotnonfinite=True)
    ...
    if interpolate:
        ...
        tmp = scipy.interpolate.RBFInterpolator(
            grP,
            c,
            kernel="linear",
            neighbors=4,
...
def bootstrapFitOI(
    ...
    if strongMJD:
        ...
        if len(MJD) < 5:  # to get at least 100 combination
            print(
                "WARNING: cannot randomise only on MJD for",
                len(MJD),
                "dates,",
        ...
        for i in range(N):
            ...
            if randomiseParam:
                ...
                if type(randomiseParam) != bool:
                    tmpfg = {
                        k: firstGuess[k] + randomiseParam * np.random.randn() * uncer[k]
                        if uncer[k] > 0
                        else firstGuess[k]
                        for k in firstGuess
                    }
                    ...
                    tmpfg = {
                        k: firstGuess[k] + np.random.randn() * uncer[k]
                        if uncer[k] > 0
                        else firstGuess[k]
                        for k in firstGuess
                    }
...
def analyseBootstrap(Boot, sigmaClipping=None, verbose=2, chi2MaxClipping=None):
    ...
    if not sigmaClipping is None:
        for k in res["fitOnly"]:
            ...
            for j in range(3):  # iterate a few times
                ...
                res["uncer"][k] = (
                    np.nanpercentile(x[tmp], 84) - np.nanpercentile(x[tmp], 16)
                ) / 2
    ...
    M = [
        [
            b["chi2"] if k == "chi2" else b["best"][k]
    ...
    res["cor"] = res["cov"] / cor
    res["covd"] = {
        ki: {kj: res["cov"][i, j] for j, kj in enumerate(fitOnly)}
        for i, ki in enumerate(fitOnly)
    }
    res["cord"] = {
        ki: {kj: res["cor"][i, j] for j, kj in enumerate(fitOnly)}
        for i, ki in enumerate(fitOnly)
    }
    if verbose:
        if not sigmaClipping is None:
            print(
                "using %d fits out of %d (sigma clipping:%.2f, chi2MaxClipping:%s)"
        ...
        for k in param.keys():
            if "line_" in k and "wl0" in k:
                ...
                if k.replace("wl0", "gaussian") in param.keys():
                    dwl = 1.5 * param[k.replace("wl0", "gaussian")] / 1000.0
                ...
                if kv in param:
                    ...
                    if kv in param:
                        if "Rin" in param:
                            ...
                            vel = param[kv] / np.sqrt(
                                0.5 * param[kv.replace("V1mas", "diamin")]
                            )
...
def _sigmaclip(x, s=4.0, n=3, maxiter=5):
    ...
    while nc and iter < maxiter:
        ...
        c = [0]
        std = (
            np.percentile(res - np.polyval(c, np.arange(N)), 84)
            - np.percentile(res - np.polyval(c, np.arange(N)), 16)
        ) / 2.0
...
def showOI(
    ...
    if not onlyMJD is None and not any(
        [mjd in onlyMJD for mjd in oi["configurations per MJD"].keys()]
    ):
        # -- nothing to plot!
        return None
    ...
    if "DPHI" in obs or "N|V|" in obs or "NV2" in obs:
        ...
        if type(param) is list:
            oi = computeDiffPhiOI(
                oi, computeLambdaParams(param[0], MJD=np.mean(oi["MJD"]))
            )
        else:
            oi = computeDiffPhiOI(
                oi, computeLambdaParams(param, MJD=np.mean(oi["MJD"]))
            )
    ...
    if not param is None:
        ...
        if type(param) == list:
            Ms = [
                VmodelOI(
                    oi, _p, imFov=imFov, imPix=imPix, imX=imX, imY=imY, debug=debug
                )
                for _p in param
            ]
            m = Ms[0]
            ...
            m = VmodelOI(
                oi, param, imFov=imFov, imPix=imPix, imX=imX, imY=imY, debug=debug
            )
            Ms = [m]
    ...
    for c, l in enumerate(obs):
        ...
        if l == "UV":  # special case for UV plot
            ...
            for e in ext:
                ...
                for k in sorted(
                    oi[e].keys(), key=lambda x: np.mean(oi[e][x]["B/wl"])
                ):  # lengh
                    # if debug:
                    ...
                    for mjd in MJDs:
                        ...
                        b = np.sqrt(oi[e][k]["u"][w] ** 2 + oi[e][k]["v"][w] ** 2)
                        try:
                            tmp = len(b)
                            bmax.extend(list(b))
                        except:
            ...
            if False and "OI_T3" in oi:
                for k in oi["OI_T3"]:
                    plt.plot(oi["OI_T3"][k]["u1"], oi["OI_T3"][k]["v1"], "1m")
                    plt.plot(-oi["OI_T3"][k]["u1"], -oi["OI_T3"][k]["v1"], "1m")
                    plt.plot(oi["OI_T3"][k]["u2"], oi["OI_T3"][k]["v2"], "2r")
                    plt.plot(-oi["OI_T3"][k]["u2"], -oi["OI_T3"][k]["v2"], "2r")
                    plt.plot(
                        oi["OI_T3"][k]["u1"] + oi["OI_T3"][k]["u2"],
        ...
        if (
            "X" in data[l]
            and spectro
            and all([data[l]["X"] in oi[data[l]["ext"]][x] for x in keys])
        ):
            keys = sorted(
                keys, key=lambda x: np.mean(oi[data[l]["ext"]][x][data[l]["X"]])
            )
        else:
            keys = sorted(keys)
        ...
        if False and l == "NFLUX" and "UV" in obs and l in obsfit:
            ...
            for k in keys:  # for each telescopes:
                ...
                for mjd in MJDs:
                    ...
                    tmp[mask] += (
                        oi[data[l]["ext"]][k][l][j, mask]
                        / oi[data[l]["ext"]][k]["E" + l][j, mask]
                    )
                    # TODO: better estimation of error of average flux
                    etmp[mask] = np.minimum(
                        etmp[mask], oi[data[l]["ext"]][k]["E" + l][j, mask]
                    )
        ...
        for i, k in enumerate(keys):
                    ...
                    if i == 0:
                        if True or len(oi["baselines"]) <= 8:
                            ...
                            if ax0 is None:
                                ...
                                ax = plt.subplot(
                                    n_flux + 1, ncol, ncol * (i_flux) + 1, sharex=ax0
                                )
                        if bckgGrid:
            ...
            if not "UV" in l and not vWl0 is None and i == 0:
                axv = ax.secondary_xaxis("top", functions=(um2kms, kms2um), color="0.6")
                axv.tick_params(
                    axis="x",
                    labelsize=5,
                    labelbottom=True,
            ...
            for mjd in MJDs:
                ...
                if (
                    "max error" in oi["fit"]
                    and data[l]["var"] in oi["fit"]["max error"]
                ):
                    # -- ignore data with large error bars
                ...
                if (
                    "max relative error" in oi["fit"]
                    and data[l]["var"] in oi["fit"]["max relative error"]
                ):
                    # -- ignore data with large error bars
                    ign *= err >= oi["fit"]["max relative error"][
                        data[l]["var"]
                    ] * np.abs(y)
                    mask *= err < oi["fit"]["max relative error"][
                        data[l]["var"]
                    ] * np.abs(y)
                    showIgn = True
                if (
                    "mult error" in oi["fit"]
                    and data[l]["var"] in oi["fit"]["mult error"]
                ):
                    err *= oi["fit"]["mult error"][data[l]["var"]]
                if (
                    "min error" in oi["fit"]
                    and data[l]["var"] in oi["fit"]["min error"]
                ):
                    err[mask] = np.maximum(
                        oi["fit"]["min error"][data[l]["var"]], err[mask]
                    )
                if (
                    "min relative error" in oi["fit"]
                    and data[l]["var"] in oi["fit"]["min relative error"]
                ):
                    # -- force error to a minimum value
                    err[mask] = np.maximum(
                        oi["fit"]["min relative error"][data[l]["var"]] * y[mask],
                        err[mask],
                    )
                # -- show data
                test = testTelescopes(k, ignoreTelescope) or testBaselines(
                ...
                if not m is None and any(mask) and data[l]["ext"] in m.keys():
                    for _im, m in enumerate(Ms):
                        ...
                        err[err <= 0] = 1
                        if "PHI" in l:
                            _resi = ((y[maskc2] - ym[maskc2] + 180) % 360 - 180) / err[
                                maskc2
                            ]
                        else:
                        ...
                        if not spectro:
                            ...
                            if not "FLUX" in l:
                                if "PHI" in l:
                                    axr.plot(
                                        X(m, j)[maskc2],
                                        ((y[maskc2] - ym[maskc2] + 180) % 360 - 180)
                                        / err[maskc2],
                                        mark,
                                        color=col,
                                        markersize=4,
                ...
                if checkImVis:
                    if not spectro:  # allInOne:
                        ax.plot(
                            X(imoi, j)[mask],
                            imoi[data[l]["ext"]][k][data[l]["var"]][j, mask]
                            + yoffset * i,
                            "1g",
                            alpha=0.5,
                        )
                ...
                if (
                    l == "DPHI" or l == "NFLUX" or l == "N|V|" or l == "NV2"
                ) and "WL cont" in oi:
                    maskc = (
                        ~oi[data[l]["ext"]][k]["FLAG"][j, :]
                        * oi["WL cont"]
                        * oi["WL mask"]
                    )
...
def showModel(
    ...
    # -- components
    comps = set(
        [k.split(",")[0].strip() for k in param.keys() if "," in k and not "&dwl" in k]
    )
    ...
    for i, wl in enumerate(imWl0):
        ...
        if axs == []:
            ...
            axs.append(
                plt.subplot(
                    1, nplot, i + 1, aspect="equal", sharex=axs[0], sharey=axs[0]
                )
            )
        ...
        if "cube" in m["MODEL"].keys():
            ...
            if np.min(im) < 0:
                ...
                print(
                    " fraction of <0 flux:",
                    "%.2e" % (-np.sum(im[im < 0]) / np.sum(im[im >= 0])),
                    end=" ",
                )
        ...
        cb = plt.colorbar(
            pc, ax=axs[-1], orientation="horizontal" if len(imWl0) > 1 else "vertical"
        )

        Xcb = np.linspace(0, 1, 5) * _imMax**imPow
        XcbL = ["%.1e" % (xcb ** (1.0 / imPow)) for xcb in Xcb]
    ...
    if (
        len(m["WL"][mask]) > 20
        and np.std(np.diff(m["WL"][mask])) / np.mean(np.diff(m["WL"][mask])) < 0.1
    ):
        plt.step(
            m["WL"][mask][w0],
            fS(m["MODEL"]["total" + key][mask][w0]),
            "-k",
            label="total",
            where="mid",
        )
    ...
    if "WL cont" in oi:
        ...
        if (
            ...
            plt.step(
                m["WL"][mask][w0],
                fS((m["MODEL"]["total" + key] * cont)[mask][w0]),
                "c",
                label="continuum",
                where="mid",
                alpha=0.7,
    ...
    for k in sorted(KZ):
        if k.endswith("," + key):
            ...
            if (
                len(m["WL"][mask]) > 20
                and np.std(np.diff(m["WL"][mask])) / np.mean(np.diff(m["WL"][mask]))
                < 0.1
            ):
                plt.step(
                    m["WL"][mask][w0],
                    fS(m["MODEL"][k][mask][w0]),
...
def halfLightRadiusFromParam(param, comp=None, fig=None, verbose=True):
    if type(param) is dict and "best" in param and "uncer" in param:
        ...
        res = {
            "best": {c: np.mean(allr[c]) for c in C},
        ...
        param = computeLambdaParams(
            param,  # MJD=np.mean(oi['MJD'])
        )

    if comp is None:
        C = filter(lambda x: x.endswith(",profile"), param.keys())
        return {
            c.split(",")[0]: halfLightRadiusFromParam(param, c.split(",")[0]) for c in C
        }
    diamin = 0
...
def halfLightRadiusFromImage(oi, icube, incl, projang, x0=None, y0=None, fig=None):
    ...
    _X = (
        (oi._model["MODEL"]["X"] - x0) * cpa - (oi._model["MODEL"]["Y"] - y0) * spa
    ) / ci + x0
    ...
    if not fig is None:
        ...
        plt.pcolormesh(
            _X, _Y, I, vmax=np.percentile(I, 99.5), shading="auto", rasterized=True
        )
        plt.title("de-rotated model")
        t = np.linspace(0, 2 * np.pi, 100)[:-1]
        plt.plot(
            rh * np.cos(t), rh * np.sin(t), ":y", alpha=0.5, label="half light radius"
        )
        ax.legend()
...
def showBootstrap(
    ...
    showP = sorted(
        filter(
            lambda k: k not in combParam.keys() and k not in ignore, boot["fitOnly"]
        ),
        key=lambda k: (
            k if not k in alternateParameterNames else alternateParameterNames[k]
        ),
    )
    showP += sorted(
        filter(lambda k: k in combParam.keys() and k not in ignore, boot["fitOnly"]),
        key=lambda k: (
            k if not k in alternateParameterNames else alternateParameterNames[k]
        ),
    )
    ...
    for i1, k1 in enumerate(showP):
        ...
        _AX[i1] = plt.subplot(len(showP), len(showP), 1 + i1 * len(showP) + i1)
        _bootAxes[k1] = _AX[i1]
        if k1 == "chi2":
            # -- assumes bins is already defined (chi2 cannot be first!)
            # -- show histogram: line and area
            h = plt.hist(
                boot["all chi2"], bins=bins, color=colorC2, histtype="step", alpha=0.9
            )
            h = plt.hist(
                boot["all chi2"],
                bins=bins,
            ...
            amps["chi2"] = 1
            plt.xlim(
                min(min(h[1]), boot["fit to all data"]["chi2"] - 0.1 * np.ptp(h[1])),
                max(max(h[1]), boot["fit to all data"]["chi2"] + 0.1 * np.ptp(h[1])),
            )
            ...
            # -- show histogram: line and area
            h = plt.hist(
                amps[k1] * (boot["all best"][k1] - offs[k1]),
                bins=bins,
                color="k",
                histtype="step",
                alpha=0.9,
            ...
            if showSingleFit and k1 in boot["fit to all data"]["best"]:
                plt.errorbar(
                    amps[k1] * (boot["fit to all data"]["best"][k1] - offs[k1]),
                    0.4 * max(h[0]),
                    markersize=fontsize / 2,
    ...
    for i1, k1 in enumerate(showP):
        for i2 in range(i1 + 1, len(showP)):
            ...
            if i1 == 0:
                _AY[i2] = plt.subplot(
                    len(showP), len(showP), 1 + i2 * len(showP) + i1, sharex=_AX[i1]
                )
                ax = _AY[i2]
            else:
                ax = plt.subplot(
            ...
            if len(boot["mask"]) < 300:
                ...
                plt.hist2d(
                    ...
                    amps[k2] * (X2 - offs[k2]),
                    cmap="bone_r",
                    bins=int(np.sqrt(len(boot["mask"]) / densify)),
                )
            ...
            t = np.linspace(0, 2 * np.pi, 100)
            sMa, sma, a = dpfit._ellParam(
                boot["cov"][_i1, _i1], boot["cov"][_i2, _i2], boot["cov"][_i1, _i2]
            )
            _X, _Y = sMa * np.cos(t), sma * np.sin(t)
            _X, _Y = _X * np.cos(a) + _Y * np.sin(a), -_X * np.sin(a) + _Y * np.cos(a)
            if (
                (k1.endswith(",x") and k2.endswith(",y"))
                or (k1.endswith(",y") and k2.endswith(",x"))
            ) and k1.split(",")[0] == k2.split(",")[0]:
                print(
                    "ellipse (emin, emax, PA) for %s/%s: %.4f %.4f %.1f"
            ...
            if (
                showSingleFit
                and k1 in boot["fit to all data"]["best"]
                and k2 in boot["fit to all data"]["best"]
            ):
                plt.plot(
                    amps[k1] * (boot["fit to all data"]["best"][k1] - offs[k1]),
...
def _Vazvar(
    ...
    if numerical:
        ...
        if numVis:
            if len(u.shape) == 2:
                Vis = Im[:, :, None, None] * np.exp(
                    -2j
                    * _c
                    * (
                        u[None, None, :, :] * X[:, :, None, None]
            ...
            Vis = np.trapezoid(np.trapezoid(Vis, axis=0), axis=0) / np.trapezoid(
                np.trapezoid(Im, axis=0), axis=0
            )
        else:
            Vis = None
    ...
    if V0 is None:
        Vis = Hankel(0) * (1.0 + 0j)  # -- force complex
    else:
        Vis = V0 * (1.0 + 0j)  # -- force complex

    for i in range(len(n)):
        if np.abs(amp[i]) > 0:
            # -- changed on 2021-10-21
            # -- checked on 2025-11-03 with stnthetic image
            Vis += (
                amp[i]
                * (-1j) ** n[i]
                * Hankel(n[i])
...
def testAzVar():
    ...
    # == image parameters =================================================
    # ~FS CMa
    thick = 0.7  # uniform disk for thick==1, uniform ring for 0<f<1
    ...
    stretch = [0.5, 90]  # eccentricity and PA of large
    ...
    if False:
        # -- uniform ring ------------------------
        _i = 1.0 * (_r <= diam / 2.0) * (_r >= diam / 2.0 * (1 - thick))
        ...
        VisR = (
            2 * scipy.special.j1(x + 1e-6) / (x + 1e-6)
            - (1 - thick) ** 2
            * 2
            * scipy.special.j1(x * (1 - thick) + 1e-6)
            / (x * (1 - thick) + 1e-6)
        ) / (1 - (1 - thick) ** 2)
    ...
    t0 = time.time()
    Vis, I, X, Y = _Vazvar(
        U / wl0, V / wl0, _i, _r, n, phi, amp, numerical=1, stretch=stretch, numVis=1
    )
    Nx = X.shape[0]
    tn = time.time() - t0
    ...
    plt.clf()
    plt.subplots_adjust(
        left=0.05, right=0.95, top=0.93, bottom=0.12, hspace=0.25, wspace=0
    )
