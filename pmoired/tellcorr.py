...
def Ftran(l, param, retWL=False, retS=False):
    ...
    if "pwv" in param.keys():
        ...
        tmpT = tran[pwvs[i0]] + (param["pwv"] - pwvs[i0]) * (
            (tran[pwvs[i0]] - tran[pwvs[i1]]) / (pwvs[i1] - pwvs[i0])
        )
    else:
        tmpT = tran80
...
def gravity(
    ...
    for i in range(4):
        if pola:
            ...
            try:
                ...
                checkExt()
                raise Exception('please use "ext=(i0,i1)" option to define OI_FLUX extensions to take into acount (2 pola)')
        else:
            if ext=='default':
                ext = 12
            try:
    ...
    if True:
        ...
        fit = dpfit.leastsqFit(
            Ftran, wl[w], p, sp[w], verbose=0, fitOnly=fitOnly, maxfev=1000
        )
        if MR:
            doNotFit.extend(["kern", "kernp", "pwv", "pow"])
        else:
            doNotFit.extend(
                ["kern_min", "kern_max", "kernp_min", "kernp_max", "pwv", "pow"]
            )
        ...
        if MR:
            ...
            doNotFit.remove("kern_min")
            doNotFit.remove("kern_max")
            doNotFit.remove("kernp_min")
            doNotFit.remove("kernp_max")
            doNotFit.remove("pwv")
            doNotFit.remove("pow")
    ...
    if save:
        ...
        c4 = fits.Column(
            name="CORR_SPEC", array=sp / Ftran(wl, fit["best"]), format="D"
        )
        c5 = fits.Column(
            name="CORR_WAVE",
            array=Ftran(wl, fit["best"], retWL=True) * 1e-6,
            format="D",
            unit="m",
        )
        c6 = fits.Column(
            name="CORR_CONT", array=Ftran(wl, fit["best"], retS=True), format="D"
        )
    ...
    plt.plot(
        wl,
        sp / Ftran(wl, p) + np.median(sp) / 8,
        "-b",
        label="telluric corrected + offset",
    )
    plt.title(filename)
    plt.ylim(
        0.9 * np.min(sp[w]),
        1.1 * np.max(sp[w] / Ftran(wl[w], p) + np.median(sp[w]) / 8),
    )
    lines = {
        "HeI": [(2.0581, 2.11274, 2.11267, 2.11258, 2.161), "c", "dashed"],
        ...
        "HI": [
            (2.1661, 2.360, 2.367, 2.374, 2.383, 2.392, 2.404, 2.416, 2.431, 2.449),
            "b",
            "dashed",
        ],
        "H2": [(2.122), "b", "dotted"],
        ...
        r"$^{12}$C$^{16}$O HB": (
            [2.2935, 2.3227, 2.3535, 2.3829, 2.4142],
            "r",
            "dashed",
        ),
        r"$^{13}$C$^{16}$O HB": (
        ...
        #'SiII':((2.200),'g'),
        "CaI": (
            (1.99318, 2.261410, 2.263110, 2.265741),
            "g",
            "dotted",
        ),  # 1.89282, 1.94508, 1.98702,
    }
